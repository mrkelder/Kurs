...
void CreateArchive() {
    ...
    do { //формирование структуры архива изделия 
...
int ReadProduct(FILE* f, ProductType* Product) {
    ...
    if (fgets(sIn, 80, f) != NULL) //считывание строки
    {
        ...
        while (token != NULL) //цикл выделения слов из исходной строки
        {
            ...
            switch (k) { //Заполнение полей структуры по номеру слова
            case 1:
...
void MakeKodifs(int* nk) {
    ...
    *nk = 0; //длина массива
    while (fgets(Sa, 80, fKodif) != NULL) //чтение "Kodif.txt"
    {
        ...
        {
            k++;
            switch (k) { //заполнение полей структуры кодификатора
            case 1:
                strcpy(Kodif.ActualKod, token);
                break;
            case 2:
                strcpy(Kodif.Name, token);
                break;
            case 3:
                strcat(strcat(Kodif.Name, " "), token);
                break;
            }
            token = strtok(NULL, Seps); //Выделение очередного слова
        }
